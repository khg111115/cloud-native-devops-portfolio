...
* stream-false: 계속 스트리밍하지 않고 통계 한 번만 반환 -> 통계를 계산하려면 비교 샘플이 필요하여 샘플링을 위한 대기가 필요
* one-shot-true: 새로운 샘플 기다리는 동작을 생략하고 즉시 사용 가능한 통계를 반환
-> 샘플링 대기시간(약 2초)가 전부 사라지면서 시간 단축
